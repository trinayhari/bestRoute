#!/usr/bin/env python3
"""
OpenRouter LLM Suite - Cost Dashboard

A dashboard for visualizing OpenRouter API usage costs and analytics.
"""
...
# Page config
st.set_page_config(
    page_title="OpenRouter LLM Suite - Cost Dashboard",
    page_icon="📊",
    layout="wide",
...
def display_sidebar():
    ...
    st.sidebar.title("Cost Dashboard")
    
    st.sidebar.markdown("### Dashboard Options")
    
    # Options could be added here if needed
    st.sidebar.info(
        "This dashboard visualizes your OpenRouter API usage and costs. "
        "It provides insights into token usage, costs, and model comparisons."
    )
    ...
    st.sidebar.markdown(
        "Data is collected from your API calls via the Cost Tracker utility. "
        "If you don't see any data, make sure you've made some API calls "
        "or generated test data using the test data generator."
    )
    ...
    st.sidebar.markdown("- **Overview**: Key metrics and trends")
    st.sidebar.markdown("- **Daily Breakdown**: Analysis by date")
    st.sidebar.markdown("- **Model Comparison**: Compare model costs")
    st.sidebar.markdown("- **Raw Data**: Access the underlying data")
...
def main():
    ...
    with tabs[2]:
        ...
        if models:
            ...
            st.markdown("Compare costs for different token counts:")
            
            # Create columns for model selection
            col1, col2, col3 = st.columns(3)
            
            with col1:
                for i, model_id in enumerate(list(models.keys())[:len(models)//3 + 1]):
                    model_name = models[model_id].get("name", model_id)
                    if st.checkbox(model_name, value=i < 3, key=f"compare_{model_id}"):
                        models_to_compare.append(model_id)
            
            with col2:
                for i, model_id in enumerate(list(models.keys())[len(models)//3 + 1:2*len(models)//3 + 1]):
                    model_name = models[model_id].get("name", model_id)
                    if st.checkbox(model_name, value=False, key=f"compare_{model_id}"):
                        models_to_compare.append(model_id)
            
            with col3:
                for i, model_id in enumerate(list(models.keys())[2*len(models)//3 + 1:]):
                    model_name = models[model_id].get("name", model_id)
                    if st.checkbox(model_name, value=False, key=f"compare_{model_id}"):
                        models_to_compare.append(model_id)
            ...
            if models_to_compare:
                ...
                for model_id in models_to_compare:
                    ...
                    costs = [(tokens * cost_per_1k) / 1000 for tokens in token_values]
                    ax.plot(token_values, costs, marker='o', label=model_name)
                ...
                ax.set_xticklabels([format_tokens(t) for t in token_values])
                ...
                st.info("Select at least one model to compare costs.")
    ...
    with tabs[3]:
        ...
        if not cost_tracker.cost_data.empty:
            ...
            # Show option to generate test data
            st.markdown("### Generate Test Data")
            st.markdown(
                "If you don't have real usage data yet, you can generate test data to see how the dashboard works. "
                "Run the following command in your terminal to generate test data:"
            )
            st.code("python generate_test_data.py")
